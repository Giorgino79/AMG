...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if options["automezzo"]:
            try:
                ...
                self.stdout.write(
                    f'🎯 Aggiornamento singolo automezzo: {options["automezzo"]}\n'
                )
            except Automezzo.DoesNotExist:
                raise CommandError(
                    f'Automezzo con targa "{options["automezzo"]}" non trovato'
                )
        else:
            automezzi = Automezzo.objects.filter(attivo=True)
            self.stdout.write(
                f"🚗 Aggiornamento tutti gli automezzi attivi: {automezzi.count()}\n"
            )
    ...
    def update_automezzo_statistics(self, automezzo):
        ...
        result = {"created": False, "updated": False, "details": []}

        # Ottieni o crea record statistiche
        stats, created = StatisticheAutomezzo.objects.get_or_create(automezzo=automezzo)
            ...
            if stats.data_aggiornamento:
                ...
                if time_diff < timedelta(hours=1):
                    result["details"].append(
                        f"⏭️  Saltato (aggiornato {time_diff.seconds//60} min fa)"
                    )
                    return result
        ...
        if consumo_info["consumo"]:
            ...
            result["details"].append(
                f'⛽ Consumo medio: {consumo_info["consumo"]:.2f} L/100km'
            )
        ...
        if costo_km_info["costo_km"]:
            ...
            result["details"].append(
                f'💰 Costo carburante: €{costo_km_info["costo_km"]:.3f}/km'
            )
        ...
        if costo_manutenzioni["costo_annuale"] is not None:
            ...
            result["details"].append(
                f'🔧 Manutenzioni/anno: €{costo_manutenzioni["costo_annuale"]:.2f}'
            )
    ...
    def calculate_consumo_medio(self, automezzo):
        ...
        rifornimenti = RifornimentoCarburante.objects.filter(
            automezzo=automezzo, data_rifornimento__gte=data_limite
        ...
        for rifornimento in rifornimenti:
            if rifornimento_precedente:
                km_percorsi = (
                    rifornimento.chilometri - rifornimento_precedente.chilometri
                )
                if km_percorsi > 0:
        ...
        if consumi:
            ...
            if end_idx > start_idx:
                ...
                result["consumo"] = Decimal(
                    str(round(sum(consumi_filtrati) / len(consumi_filtrati), 2))
                )
    ...
    def calculate_costo_per_km(self, automezzo):
        ...
        if (
            rifornimenti_data["costo_totale"]
            and rifornimenti_data["primo_km"]
            and rifornimenti_data["ultimo_km"]
        ):
    ...
    def calculate_costo_manutenzioni_annuali(self, automezzo):
        ...
        ).aggregate(costo_totale=Sum("costo_effettivo"), count=Count("id"))
    ...
    def get_ultimo_rifornimento(self, automezzo):
        ...
        ultimo = (
            RifornimentoCarburante.objects.filter(automezzo=automezzo)
            .order_by("-data_rifornimento")
            .first()
        )
    ...
    def get_ultima_manutenzione(self, automezzo):
        ...
        ultima = (
            Manutenzione.objects.filter(automezzo=automezzo, completata=True)
            .order_by("-data_effettuata")
            .first()
        )
    ...
    def calculate_additional_statistics(self, automezzo):
        ...
        eventi_count = (
            EventoAutomezzo.objects.filter(automezzo=automezzo)
            .values("tipo")
    ...
    def print_summary(self, processed, created, updated, errors):
        ...
        self.stdout.write(
            f"⏭️  Nessuna modifica: {processed - created - updated - errors}"
        )
        ...
        if self.dry_run:
            self.stdout.write(
                self.style.WARNING(
                    "🔍 MODALITÀ DRY-RUN: Nessuna modifica è stata salvata"
                )
            )
