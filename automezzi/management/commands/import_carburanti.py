...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--file", type=str, help="Path al file da importare (CSV, JSON, XML)"
        )
        parser.add_argument(
            "--format",
            type=str,
        ...
        parser.add_argument(
            "--api-url", type=str, help="URL API per importazione automatica prezzi"
        )
        parser.add_argument(
            "--update-existing",
            action="store_true",
        ...
        parser.add_argument(
            "--backup", action="store_true", help="Crea backup prima dell'importazione"
        )
        parser.add_argument(
            "--preset",
            type=str,
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS("⛽ Import Carburanti e Prezzi...\n"))
    ...
    def detect_file_format(self, filepath):
        ...
        format_map = {".csv": "csv", ".json": "json", ".xml": "xml"}
        ...
        for encoding in encodings:
            try:
                with open(filepath, "r", encoding=encoding) as csvfile:
                    ...
                    self.stdout.write(
                        f"  ✓ {len(data)} righe caricate (encoding: {encoding})"
                    )
                    return data
    ...
    def parse_csv_row(self, row):
        ...
        return {"nome": nome, "costo_per_litro": prezzo}
    ...
    def load_json_file(self, filepath):
        ...
        for item in data:
            if "nome" in item and "costo_per_litro" in item:
                ...
                normalized_data.append(
                    {"nome": item["name"], "costo_per_litro": float(item["price"])}
                )
    ...
    def load_xml_file(self, filepath):
        ...
        for fuel_elem in root.findall(".//carburante") or root.findall(".//fuel"):
            ...
            prezzo = (
                fuel_elem.find("prezzo")
                or fuel_elem.find("price")
                or fuel_elem.find("costo_per_litro")
            )
        ...
        with transaction.atomic():
            for fuel_data in data:
                try:
                    ...
                    self.stdout.write(
                        self.style.ERROR(
                            f'  ❌ Errore {fuel_data.get("nome", "?")}: {str(e)}'
                        )
                    )
    ...
    def process_fuel(self, fuel_data):
        ...
        try:
            ...
            if self.create_missing:
                TipoCarburante.objects.create(nome=nome, costo_per_litro=prezzo)
                return "created"
            else:
                return "skipped"
    ...
    def print_import_summary(self, stats):
        ...
        if self.dry_run:
            self.stdout.write(
                self.style.WARNING("🔍 MODALITÀ DRY-RUN: Nessuna modifica salvata")
            )
