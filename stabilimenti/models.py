...
class Stabilimento(AllegatiMixin, SearchMixin, models.Model):
    """
    ...
    nome = models.CharField(
        max_length=200, help_text="Nome identificativo dello stabilimento"
    )
    ...
    cap = models.CharField(
        max_length=5,
        validators=[
            RegexValidator(regex=r"^\d{5}$", message="Il CAP deve essere di 5 cifre")
        ],
    )
    ...
    email_filiale = models.EmailField(
        blank=True, null=True, help_text="Email principale dello stabilimento"
    )
    ...
    superficie_mq = models.PositiveIntegerField(
        blank=True, null=True, help_text="Superficie in metri quadrati"
    )
    ...
    anno_costruzione = models.PositiveSmallIntegerField(
        blank=True, null=True, help_text="Anno di costruzione dell'edificio"
    )
    ...
    data_chiusura = models.DateField(
        blank=True, null=True, help_text="Data di chiusura (se applicabile)"
    )

    # === NOTE ===
    note_generali = models.TextField(
        blank=True, null=True, help_text="Note generali sullo stabilimento"
    )
    ...
    def get_search_fields(cls):
        ...
        return ['nome', 'codice_stabilimento', 'indirizzo', 'citta', 'provincia', 'telefono']
    ...
    def get_absolute_url(self):
        """URL per navigare al dettaglio dello stabilimento"""
    ...
    def get_costi_anno_corrente(self):
        ...
        return self.costi.filter(data_fattura__year=anno_corrente).aggregate(
...
class CostiStabilimentoManager(models.Manager):
    ...
    def del_periodo(self, data_inizio, data_fine):
        ...
        return self.filter(data_fattura__gte=data_inizio, data_fattura__lte=data_fine)


class CostiStabilimento(AllegatiMixin, SearchMixin, models.Model):
    """
    ...
    numero_pratica = models.CharField(
        max_length=50, unique=True, help_text="Numero pratica generato automaticamente"
    )
    ...
    titolo = models.CharField(
        max_length=200, help_text="Titolo/oggetto dell'intervento o servizio"
    )
    descrizione = models.TextField(help_text="Descrizione dettagliata dell'intervento")
    ...
    data_richiesta = models.DateField(
        default=timezone.now, help_text="Data della richiesta/preventivo"
    )
    data_inizio_lavori = models.DateField(
        blank=True, null=True, help_text="Data inizio intervento/servizio"
    )
    data_fine_lavori = models.DateField(
        blank=True, null=True, help_text="Data fine intervento/servizio"
    )
    data_fattura = models.DateField(
        blank=True, null=True, help_text="Data della fattura"
    )
    ...
    note_interne = models.TextField(
        blank=True, null=True, help_text="Note interne non visibili nei documenti"
    )
    ...
    consumo_mc = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True
    )
    ...
    def get_search_fields(cls):
        """Campi ricercabili nella ricerca globale"""
        return ['numero_pratica', 'titolo', 'descrizione']

    def get_search_result_display(self):
        """Testo visualizzato nei risultati di ricerca"""
        return f"{self.numero_pratica} - {self.titolo}"

    def get_absolute_url(self):
        """URL per navigare al dettaglio del costo"""
        from django.urls import reverse
...
class DocStabilimento(SearchMixin, models.Model):
    """
    ...
    class TipoDocumento(models.TextChoices):
        ...
        CONTRATTO_DERATTIZZAZIONE = (
            "contratto_derattizzazione",
            "Contratto Derattizzazione",
        )
        CONTRATTO_DISINFESTAZIONE = (
            "contratto_disinfestazione",
    ...
    nome_documento = models.CharField(
        max_length=200, help_text="Nome identificativo del documento"
    )
    ...
    versione = models.CharField(
        max_length=10, default="1.0", help_text="Versione del documento"
    )

    # === CONTENUTO ===
    descrizione = models.TextField(
        blank=True, null=True, help_text="Descrizione del contenuto del documento"
    )
    file_documento = models.FileField(
        upload_to="documenti_stabilimenti/%Y/%m/", help_text="File del documento"
    )
    ...
    # === STATO ===
    attivo = models.BooleanField(
        default=True, help_text="Indica se il documento è ancora valido/attivo"
    )

    # === NOTE ===
    note = models.TextField(
        blank=True, null=True, help_text="Note aggiuntive sul documento"
    )
    ...
    def get_search_fields(cls):
        """Campi ricercabili nella ricerca globale"""
        return ['nome_documento', 'descrizione', 'versione']
    ...
    def get_absolute_url(self):
        """URL per navigare al dettaglio del documento"""
        from django.urls import reverse
        return reverse('stabilimenti:documento_detail', kwargs={'pk': self.pk})
