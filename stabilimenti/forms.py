...
class StabilimentoForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "indirizzo": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "Via, numero civico"}
            ),
            "cap": forms.TextInput(
                attrs={
                    "class": "form-control",
            ...
            "citta": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "Città"}
            ),
            ...
            "telefono": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "+39 06 12345678"}
            ),
            "email_filiale": forms.EmailInput(
                attrs={
                    "class": "form-control",
                    "placeholder": "stabilimento@azienda.it",
                }
            ),
            "responsabile_operativo": forms.Select(attrs={"class": "form-select"}),
            "responsabile_amministrativo": forms.Select(attrs={"class": "form-select"}),
            "superficie_mq": forms.NumberInput(
                attrs={"class": "form-control", "min": "1"}
            ),
            "numero_piani": forms.NumberInput(
                attrs={"class": "form-control", "min": "1", "max": "50"}
            ),
            "anno_costruzione": forms.NumberInput(
                attrs={
                    "class": "form-control",
                    "min": "1800",
    ...
    def __init__(self, *args, user=None, **kwargs):
        ...
        # Empty labels
        self.fields["responsabile_operativo"].empty_label = (
            "Seleziona responsabile operativo"
        )
        self.fields["responsabile_amministrativo"].empty_label = (
            "Seleziona responsabile amministrativo"
        )
        ...
        # Help text
        self.fields["responsabile_operativo"].help_text = (
            "Responsabile delle operazioni quotidiane"
        )
        self.fields["responsabile_amministrativo"].help_text = (
            "Responsabile degli aspetti amministrativi"
        )
        ...
        self.helper.layout = Layout(
            ...
            Fieldset(
                ...
                Row(
                    ...
                    (
                        Column("codice_stabilimento", css_class="form-group col-md-4")
                        if self.instance.pk
                        else None
                    ),
                ),
            ...
            Fieldset(
                ...
                Row(
                    ...
                    Column(
                        "responsabile_amministrativo", css_class="form-group col-md-6"
                    ),
                ),
            ),
            Fieldset(
            ...
            Fieldset("Note", "note_generali"),
        )

        # Rimuovi campi None dal layout
        self.helper.layout.fields = [
            f for f in self.helper.layout.fields if f is not None
        ]
        ...
        if anno:
            ...
            if anno < 1800 or anno > anno_corrente:
                raise ValidationError(
                    f"L'anno deve essere compreso tra 1800 e {anno_corrente}"
                )
        return anno
...
class CostiStabilimentoForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            "iva_percentuale": forms.NumberInput(
                attrs={
                    "class": "form-control",
                    "step": "0.01",
                    "min": "0",
            ...
            "data_richiesta": forms.DateInput(
                attrs={"class": "form-control", "type": "date"}
            ),
            "data_inizio_lavori": forms.DateInput(
                attrs={"class": "form-control", "type": "date"}
            ),
            "data_fine_lavori": forms.DateInput(
                attrs={"class": "form-control", "type": "date"}
            ),
            "data_fattura": forms.DateInput(
                attrs={"class": "form-control", "type": "date"}
            ),
            "data_scadenza_servizio": forms.DateInput(
                attrs={"class": "form-control", "type": "date"}
            ),
            "preventivo": forms.FileInput(
                attrs={
                    "class": "form-control",
                    "accept": ".pdf,.doc,.docx,.jpg,.jpeg,.png",
                }
            ),
            "fattura": forms.FileInput(
                attrs={
                    "class": "form-control",
                    "accept": ".pdf,.doc,.docx,.jpg,.jpeg,.png",
                }
            ),
            "certificato": forms.FileInput(
                attrs={
                    "class": "form-control",
                    "accept": ".pdf,.doc,.docx,.jpg,.jpeg,.png",
                }
    ...
    def __init__(self, *args, user=None, stabilimento=None, **kwargs):
        ...
        # Configura queryset
        self.fields["stabilimento"].queryset = Stabilimento.objects.attivi().order_by(
            "nome"
        )
        ...
        # Help text
        self.fields["data_scadenza_servizio"].help_text = (
            "Data del prossimo intervento o scadenza certificazione"
        )
    ...
    def clean(self):
        ...
        if data_inizio and data_fine and data_inizio > data_fine:
            raise ValidationError(
                "La data di fine lavori non può essere precedente all'inizio"
            )

        # Verifica data richiesta
        if data_richiesta and data_inizio and data_richiesta > data_inizio:
            raise ValidationError(
                "La data di richiesta non può essere successiva all'inizio lavori"
            )
...
class DocStabilimentoForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            "stabilimento": forms.Select(attrs={"class": "form-select"}),
            "nome_documento": forms.TextInput(
                attrs={
                    "class": "form-control",
            ...
            "versione": forms.TextInput(
                attrs={"class": "form-control", "placeholder": "1.0"}
            ),
            "descrizione": forms.Textarea(
                attrs={
                    "class": "form-control",
            ...
            "file_documento": forms.FileInput(
                attrs={
                    "class": "form-control",
    ...
    def __init__(self, *args, user=None, stabilimento=None, **kwargs):
        ...
        self.fields["data_documento"].help_text = (
            "Data del documento (se diversa da oggi)"
        )
        self.fields["data_scadenza"].help_text = (
            "Data di scadenza del documento (se applicabile)"
        )
    ...
    def clean_data_scadenza(self):
        ...
        if data_scadenza:
            if data_documento and data_scadenza < data_documento:
                raise ValidationError(
                    "La data di scadenza non può essere precedente alla data del documento"
                )
...
class StabilimentiSearchForm(forms.Form):
    ...
    provincia = forms.CharField(
        ...
        required=False,
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "RM", "maxlength": "2"}
        ),
    )
...
class CostiSearchForm(forms.Form):
    ...
    causale = forms.ChoiceField(
        choices=[("", "Tutte le tipologie")]
        + list(CostiStabilimento.TipoCosto.choices),
        required=False,
        widget=forms.Select(attrs={"class": "form-select"}),
    )
...
class UtenzaForm(CostiStabilimentoForm):
    ...
    class Meta(CostiStabilimentoForm.Meta):
        ...
        widgets = {
            **CostiStabilimentoForm.Meta.widgets,  # Eredita tutti i widget esistenti
            ...
            "periodo_fatturazione_da": forms.DateInput(
                attrs={"class": "form-control", "type": "date"}
            ),
            "periodo_fatturazione_a": forms.DateInput(
                attrs={"class": "form-control", "type": "date"}
            ),
            "codice_pdr_pod": forms.TextInput(
                attrs={
                    "class": "form-control",
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields["codice_pdr_pod"].help_text = (
            "Codice identificativo utenza (PDR per gas, POD per luce)"
        )
        ...
        self.fields["titolo"].widget.attrs[
            "placeholder"
        ] = "Es: Bolletta Enel Gennaio 2025"
        self.fields["data_scadenza_servizio"].label = "Prossima Lettura"
        self.fields["data_scadenza_servizio"].help_text = (
            "Data della prossima lettura o scadenza contratto"
        )
    ...
    def clean_periodo_fatturazione_a(self):
        ...
        if periodo_da and periodo_a and periodo_da > periodo_a:
            raise ValidationError(
                "Il periodo di fine non può essere precedente all'inizio"
            )
