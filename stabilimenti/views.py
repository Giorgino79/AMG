...
from .forms import (
    StabilimentoForm,
    CostiStabilimentoForm,
    DocStabilimentoForm,
    StabilimentiSearchForm,
    CostiSearchForm,
    UtenzaForm,
)
...
def stabilimenti_access_required(user):
    ...
    if hasattr(user, "livello") and user.livello in [
        "amministratore",
        "contabile",
        "operativo",
    ]:
...
def stabilimenti_list(request):
    ...
    context = {
        ...
        "breadcrumbs": [{"name": "Stabilimenti", "url": None}],
    }
...
def dettaglio_stabilimento(request, pk):
    ...
    # Costi recenti
    costi_recenti = stabilimento.costi.select_related("fornitore").order_by(
        "-data_creazione"
    )[:5]
    ...
    # ContentType per allegati e QR code
    from django.contrib.contenttypes.models import ContentType
    content_type_id = ContentType.objects.get_for_model(Stabilimento).id

    context = {
        "stabilimento": stabilimento,
        "object": stabilimento,  # Alias per template comune
        ...
        "back_url": reverse("stabilimenti:list"),
        "edit_url": reverse("stabilimenti:modifica", kwargs={"pk": pk}),
        "breadcrumbs": [
            {"name": "Stabilimenti", "url": reverse("stabilimenti:list")},
...
def modifica_stabilimento(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                messages.success(
                    request,
                    f'Stabilimento "{stabilimento.nome}" modificato con successo',
                )
    ...
    context = {
        ...
        "breadcrumbs": [
            {"name": "Stabilimenti", "url": reverse("stabilimenti:list")},
            {
                "name": stabilimento.nome,
                "url": reverse("stabilimenti:dettaglio", kwargs={"pk": pk}),
            },
...
def costi_list(request):
    ...
    # Query base
    costi = CostiStabilimento.objects.select_related(
        "stabilimento", "fornitore", "incaricato"
    )
...
def nuovo_costo(request, stabilimento_pk):
    ...
    if request.method == "POST":
        form = CostiStabilimentoForm(
            request.POST, request.FILES, user=request.user, stabilimento=stabilimento
        )
    ...
    context = {
        ...
        "breadcrumbs": [
            {"name": "Stabilimenti", "url": reverse("stabilimenti:list")},
            {
                "name": stabilimento.nome,
                "url": reverse(
                    "stabilimenti:dettaglio", kwargs={"pk": stabilimento.pk}
                ),
            },
...
def dettaglio_costo(request, pk):
    ...
    costo = get_object_or_404(
        CostiStabilimento.objects.select_related(
            "stabilimento", "fornitore", "incaricato"
        ),
        pk=pk,
    )
    ...
    context = {
        ...
        "breadcrumbs": [
            {"name": "Stabilimenti", "url": reverse("stabilimenti:list")},
            {
                "name": costo.stabilimento.nome,
                "url": reverse(
                    "stabilimenti:dettaglio", kwargs={"pk": costo.stabilimento.pk}
                ),
            },
...
def modifica_costo(request, pk):
    ...
    if request.method == "POST":
        form = CostiStabilimentoForm(
            request.POST, request.FILES, instance=costo, user=request.user
        )
        ...
        if form.is_valid():
            try:
                ...
                messages.success(
                    request, f'Costo "{costo.titolo}" modificato con successo'
                )
                return redirect("stabilimenti:dettaglio_costo", pk=pk)
    ...
    context = {
        ...
        "breadcrumbs": [
            ...
            {
                "name": f"Costo {costo.numero_pratica}",
                "url": reverse("stabilimenti:dettaglio_costo", kwargs={"pk": pk}),
            },
            {"name": "Modifica", "url": None},
        ],
    }
    ...
    context = {
        ...
        "breadcrumbs": [
            {"name": "Stabilimenti", "url": reverse("stabilimenti:list")},
            {
                "name": stabilimento.nome,
                "url": reverse(
                    "stabilimenti:dettaglio", kwargs={"pk": stabilimento_pk}
                ),
            },
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                messages.success(
                    request,
                    f'Documento "{documento.nome_documento}" caricato con successo',
                )
                return redirect(
                    "stabilimenti:documenti", stabilimento_pk=stabilimento_pk
                )
    ...
    context = {
        ...
        "breadcrumbs": [
            ...
            },
            {
                "name": "Documenti",
                "url": reverse(
                    "stabilimenti:documenti",
                    kwargs={"stabilimento_pk": stabilimento_pk},
                ),
            },
...
def scadenze_dashboard(request):
    ...
    scadute = DocStabilimento.objects.filter(
        data_scadenza__lt=oggi, attivo=True
    ...
    tutte_scadenze = DocStabilimento.objects.filter(
        data_scadenza__isnull=False, attivo=True
    ).count()

    scadenze_future = DocStabilimento.objects.filter(
        data_scadenza__gte=oggi, attivo=True
    ).count()
...
def utenze_stabilimento(request, stabilimento_pk):
    ...
    utenze = (
        CostiStabilimento.objects.filter(
            stabilimento=stabilimento, causale__in=utenze_types
        )
        .select_related("fornitore", "incaricato")
    ...
    for tipo_code, tipo_display in CostiStabilimento.TipoCosto.choices:
        if tipo_code in utenze_types:
            costi_tipo = utenze.filter(
                causale=tipo_code, data_fattura__year=anno_corrente
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            try:
                ...
                messages.success(
                    request, f'Utenza "{utenza.titolo}" creata con successo'
                )
                return redirect(
                    "stabilimenti:utenze_stabilimento", stabilimento_pk=stabilimento_pk
                )
    ...
    context = {
        ...
        "breadcrumbs": [
            ...
            },
            {
                "name": "Utenze",
                "url": reverse(
                    "stabilimenti:utenze_stabilimento",
                    kwargs={"stabilimento_pk": stabilimento_pk},
                ),
            },
...
def modifica_utenza(request, pk):
    ...
    if request.method == "POST":
        form = UtenzaForm(
            request.POST, request.FILES, instance=utenza, user=request.user
        )
        ...
        if form.is_valid():
            try:
                ...
                messages.success(
                    request, f'Utenza "{utenza.titolo}" modificata con successo'
                )
                return redirect(
                    "stabilimenti:utenze_stabilimento",
                    stabilimento_pk=utenza.stabilimento.pk,
                )
    ...
    context = {
        ...
        "breadcrumbs": [
            {"name": "Stabilimenti", "url": reverse("stabilimenti:list")},
            {
                "name": utenza.stabilimento.nome,
                "url": reverse(
                    "stabilimenti:dettaglio", kwargs={"pk": utenza.stabilimento.pk}
                ),
            },
            {
                "name": "Utenze",
                "url": reverse(
                    "stabilimenti:utenze_stabilimento",
                    kwargs={"stabilimento_pk": utenza.stabilimento.pk},
                ),
            },
...
def dashboard_utenze(request):
    ...
    # Query base utenze
    utenze = CostiStabilimento.objects.filter(causale__in=utenze_types).select_related(
    ...
    for stabilimento in Stabilimento.objects.attivi():
        costo_stabilimento = utenze.filter(
            stabilimento=stabilimento, data_fattura__year=anno_corrente
