...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        documenti_in_scadenza = (
            DocStabilimento.objects.filter(
                data_scadenza__gte=oggi, data_scadenza__lte=data_limite, attivo=True
            )
        ...
        if dry_run:
            self.stdout.write(self.style.WARNING("DRY-RUN: Nessun promemoria creato."))
            return

        # Crea promemoria
        promemoria_creati = self._crea_promemoria(
            documenti_in_scadenza, destinatari, giorni_anticipo
        )
    ...
    def _get_destinatari_promemoria(self):
        ...
        if hasattr(User, "livello"):
            destinatari.extend(
                User.objects.filter(
                    livello__in=["amministratore", "contabile"], is_active=True
                )
            )

        # Fallback: cerca staff e superuser
        if not destinatari:
            destinatari.extend(User.objects.filter(is_staff=True, is_active=True))
